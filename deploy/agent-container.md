...
export TOKEMON_INGEST_TOKEN='replace-with-a-generated-secret'
...
`$HOME`. The agent writes cursors only to `TOKEMON_AGENT_STATE_DIR` (default
`./data/agent-state`), which is the sole writable mount. If your deployment
cannot provide read-only mounts, do not run an agent container; use the native
